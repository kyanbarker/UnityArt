...
public class Script2 : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        GameObject CreatePrismY(string name = "Prism")
        {
            ...
            TransformController transformController = prism.AddComponent<TransformController>();

            // lerp the y rotation between -180 and 180 using a linear wave over 16 seconds
            void SetupYRotationController()
            {
                LFOFloatTarget target = prism.AddComponent<LFOFloatTarget>();
                target.Min = -180f;
                target.Max = 180f;
                target.Action = yRotation =>
                {
                    transformController.RotationY = yRotation;
                };
                LFOController controller = prism.AddComponent<LFOController>();
                controller.Waveform = linearWaveform;
                controller.Frequency = Random.Range(1 / 16f, 1 / 8f);
                controller.Targets = new LFOTarget[] { target };
            }
            SetupYRotationController();
            return prism;
        }
        ...
        GameObject CreatePrism(string name = "Prism")
        {
            ...
            GameObject CreatePlane(string name = "Plane")
            {
                ...
                GameObject linePrefab = Resources.Load<GameObject>("Scene2/Line");

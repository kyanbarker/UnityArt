...
public class ComponentMigrationTool : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (dryRun)
        {
            EditorGUILayout.HelpBox("Dry Run Mode: No changes will be made. Check console for preview.", MessageType.Info);
        }
        else if (removeOldComponents)
        {
            EditorGUILayout.HelpBox("WARNING: Old components will be removed! Make sure to backup your project.", MessageType.Warning);
        }
        ...
        EditorGUILayout.LabelField($"Found {copyTransformComponents.Length} CopyAndTransform1 components");
    ...
    private void MigrateLFOToLFO2(LFO[] lfoComponents)
    {
        if (dryRun)
        {
            ...
            {
                Debug.Log($"  - {lfo.gameObject.name}: waveform={lfo.waveform}, bpm={lfo.BPM}, beatsPerCycle={lfo.BeatsPerCycle}");
            }
            return;
        }
        ...
        foreach (var oldLFO in lfoComponents)
        {
            ...
            {
                Debug.LogWarning($"GameObject {gameObject.name} already has LFO2 component. Skipping.");
                continue;
            }
            ...
            // Copy waveform
            newSO.FindProperty("waveform").enumValueIndex = oldSO.FindProperty("waveform").enumValueIndex;
            
            // Copy customCurve
            newSO.FindProperty("customCurve").animationCurveValue = oldSO.FindProperty("customCurve").animationCurveValue;
            
            // Copy phaseOffset
            newSO.FindProperty("phaseOffset").floatValue = oldSO.FindProperty("phaseOffset").floatValue;
            
            // Copy BPM-related fields (now inherited from BPMController → CycleInBeatsController)
            newSO.FindProperty("useExternalBPMTime").boolValue = oldSO.FindProperty("useExternalBPM").boolValue;
            newSO.FindProperty("externalBPMTime").objectReferenceValue = oldSO.FindProperty("externalBPM").objectReferenceValue;
            ...
            newSO.FindProperty("beatsPerCycle").floatValue = oldSO.FindProperty("beatsPerCycle").floatValue;
    ...
    private void MigrateCopyAndTransform1ToCopyAndTransform2(CopyAndTransform1[] copyTransformComponents)
    {
        if (dryRun)
        {
            Debug.Log($"[DRY RUN] Would migrate {copyTransformComponents.Length} CopyAndTransform1 components:");
            foreach (var ct in copyTransformComponents)
            {
                ...
                {
                    case RotationAxis.X: deltaRotation = new Vector3(rotationDegrees, 0, 0); break;
                    case RotationAxis.Y: deltaRotation = new Vector3(0, rotationDegrees, 0); break;
                    case RotationAxis.Z: deltaRotation = new Vector3(0, 0, rotationDegrees); break;
                }
                
                Debug.Log($"  - {ct.gameObject.name}: rotation {rotationDegrees}° on {(RotationAxis)rotationAxisIndex} → {deltaRotation}, numCopies={oldSO.FindProperty("numCopies").intValue}, delayBeats={oldSO.FindProperty("delayBeats").floatValue}");
            }
            return;
        }
        ...
        foreach (var oldCopyTransform in copyTransformComponents)
        {
            ...
            {
                Debug.LogWarning($"GameObject {gameObject.name} already has CopyAndTransform2 component. Skipping.");
                continue;
            }
            ...
            newSO.FindProperty("deltaPosition").vector3Value = oldSO.FindProperty("deltaPosition").vector3Value;
            newSO.FindProperty("deltaScale").vector3Value = oldSO.FindProperty("deltaScale").vector3Value;
            ...
            cycleBeatsSO.FindProperty("useExternalBPMTime").boolValue = oldSO.FindProperty("useParentBPM").boolValue;
            ...
            cycleBeatsSO.FindProperty("beatsPerCycle").floatValue = oldSO.FindProperty("delayBeats").floatValue;
            ...
            Debug.Log($"Added CycleInBeatsController + OnNewCycleController with beatsPerCycle={oldSO.FindProperty("delayBeats").floatValue} and wired to CreateCopy");

            migratedCount++;
            Debug.Log($"Migrated CopyAndTransform1 → CopyAndTransform2 on {gameObject.name}");
            Debug.Log($"  Converted rotation: {rotationDegrees}° on {(RotationAxis)rotationAxisIndex} axis → {deltaRotation}");
        ...
        Debug.Log($"Successfully migrated {migratedCount} CopyAndTransform1 components to CopyAndTransform2");
    }
}
...
public static class MigrationUtilities
{
    ...
    public static void MigrateSelectedLFOComponents()
    {
        ...
        Debug.Log($"Found {lfoComponents.Count} LFO components in selection. Use the Migration Tool window for full control.");
    }
    ...
    public static void MigrateSelectedCopyAndTransformComponents()
    {
        ...
        Debug.Log($"Found {copyTransformComponents.Count} CopyAndTransform1 components in selection. Use the Migration Tool window for full control.");
    }
